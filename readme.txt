Script currently only do two things : it can reset the database ; and it can collect all the data from all recorded circles.
...
## tests :
Two tests have been made :
- test with only one a circle+ (short address, not tested with long addresses) ;
- test retrieving data from one circle through a circle+ (long addresses).

In the second case, error messages appear from the plugwise plugin when initializing the perl object ; I think the messages come from the circle associated to the circle+.
